...
- allows it to track the full history (provenance) of data
...
believe they are *useful* and will allow *composable* software to be reused and
will advance the current state of bioinformatics.

Accessibility and Transferability
---------------------------------
QIIME 2 stores all data as a *directory structure* inside a ZIP file.
...
Furthermore, because some common formats are used on a per-sample basis,
the data that may be consumed by a tool would be a *directory* of formats.
...
Some that manipulate ZIP files directly
(often built into an OS GUI)
and others that use ZIP files as a backing structure
(such as ``.docx`` and ``.epub``),
making long-term accessibility more likely.
...
We need data about the data, or *metadata*
...
Learn More About Type Checking in QIIME 2
`````````````````````````````````````````
...
which instructs the computer what the specific layout of ``/data/`` is. Once this is
known, it is possible to convert that data into other formats.
...
which is recorded in ``metadata.yaml`` and defines a layout within a directory
built with file :term:`formats<Format>`.
